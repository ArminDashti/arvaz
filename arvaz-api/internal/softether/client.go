...
type HubUser struct {
	...
	TrafficYesterdaySeconds int64  `json:"trafficYesterdaySeconds"`
	TrafficWeekSeconds      int64  `json:"trafficWeekSeconds"`
	TrafficMonthSeconds     int64  `json:"trafficMonthSeconds"`
}
...
func (c *Client) vpncmd(ctx context.Context, args ...string) (string, error) {
	...
	c.killOrphanedVpncmd()

	// Prefer localhost admin without /PASSWORD (T3 deadlocks when /PASSWORD is set
	// under admin socket contention). Fall back to password auth for other hosts.
	out, err := c.runVpncmdLocked(ctx, false, args...)
	if err != nil && c.Password != "" && vpncmdAuthRetryable(err) {
		c.killOrphanedVpncmd()
		return c.runVpncmdLocked(ctx, true, args...)
	}
	return out, err
}

func vpncmdAuthRetryable(err error) bool {
	if err == nil {
		return false
	}
	msg := strings.ToLower(err.Error())
	return strings.Contains(msg, "auth failed") ||
		strings.Contains(msg, "access has been denied") ||
		strings.Contains(msg, "password:")
}

func (c *Client) runVpncmdLocked(ctx context.Context, withPassword bool, args ...string) (string, error) {
...
// killOrphanedVpncmd stops vpncmd left running when docker exec is cancelled.
// softether shares mullvad-1's network namespace, so orphans often appear on the
// host PID tree rather than inside the container's ps output.
func (c *Client) killOrphanedVpncmd() {
	ctx, cancel := context.WithTimeout(context.Background(), 8*time.Second)
	defer cancel()
	script := `pids=$(ps -C vpncmd -o pid= 2>/dev/null | tr -d " "); [ -n "$pids" ] && kill -TERM $pids 2>/dev/null; sleep 1; pids=$(ps -C vpncmd -o pid= 2>/dev/null | tr -d " "); [ -n "$pids" ] && kill -KILL $pids 2>/dev/null; true`
	for _, container := range []string{c.Container, "mullvad-1"} {
		cmd := exec.CommandContext(ctx, "docker", "exec", container, "sh", "-c", script)
		_ = cmd.Run()
	}
	// Host-visible orphans (shared netns): pid namespace of mullvad-1 / softether stack.
		hostKill := exec.CommandContext(ctx, "docker", "run", "--rm", "--pid", "host",
		"alpine:3.21", "sh", "-c", script)
	_ = hostKill.Run()
}
