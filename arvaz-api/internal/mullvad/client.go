...
func (c *Client) Status(ctx context.Context) (*Status, error) {
	...
	tunnelRaw, _ := c.exec(ctx, 15*time.Second, "mullvad", "tunnel", "get")
	qr, daita := parseTunnelFlags(tunnelRaw)
	st := &Status{
	...
	if m := reFeat.FindStringSubmatch(raw); len(m) == 2 {
		...
		// Prefer tunnel get; fall back to Features line when tunnel get failed.
		if strings.TrimSpace(tunnelRaw) == "" {
			feat := strings.ToLower(st.Features)
			st.QuantumResistant = strings.Contains(feat, "quantum")
			st.Daita = strings.Contains(feat, "daita")
		}
	}
	return st, nil
}
...
func (c *Client) SetTunnel(ctx context.Context, quantumResistant, daita *bool) error {
	if quantumResistant == nil && daita == nil {
		return fmt.Errorf("no tunnel options provided")
	}
	if quantumResistant != nil {
		val := "off"
		if *quantumResistant {
			val = "on"
		}
		if _, err := c.exec(ctx, 30*time.Second, "mullvad", "tunnel", "set", "quantum-resistant", val); err != nil {
			return err
		}
	}
	if daita != nil {
		...
		if _, err := c.exec(ctx, 30*time.Second, "mullvad", "tunnel", "set", "daita", val); err != nil {
			return err
		}
	}
	return nil
...
var (
	reQuantumResistance = regexp.MustCompile(`(?i)Quantum\s+resistance:\s*(\S+)`)
	reDaita             = regexp.MustCompile(`(?i)DAITA:\s*(\S+)`)
)

func parseTunnelFlags(raw string) (quantumResistant, daita bool) {
	if m := reQuantumResistance.FindStringSubmatch(raw); len(m) == 2 {
		quantumResistant = tunnelFlagOn(m[1])
	}
	if m := reDaita.FindStringSubmatch(raw); len(m) == 2 {
		daita = tunnelFlagOn(m[1])
	}
	return quantumResistant, daita
}

func tunnelFlagOn(v string) bool {
