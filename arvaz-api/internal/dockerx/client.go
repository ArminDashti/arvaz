...
func (c *Client) ListContainers(ctx context.Context) ([]ContainerInfo, error) {
	...
	for _, row := range inspected {
		...
		disk := containerDiskBytes(row.SizeRootFs, row.SizeRw)
		...
		result = append(result, ContainerInfo{
			...
			DiskBytes:     disk,
			DiskGB:        round2(float64(disk) / (1024 * 1024 * 1024)),
			Network:       network,
			IPPort:        ipPort,
			HAProxyURL:    haproxy,
...
func containerDiskBytes(sizeRootFs, sizeRw int64) uint64 {
	if sizeRootFs > 0 {
		return uint64(sizeRootFs)
	}
	if sizeRw > 0 {
