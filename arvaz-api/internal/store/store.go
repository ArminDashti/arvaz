...
type UserTrafficPeriods struct {
	...
	YesterdaySeconds int64  `json:"yesterdaySeconds"`
...
// ListHubUsersFromStats builds SE Users rows from PostgreSQL when live vpncmd is unavailable.
func (s *Store) ListHubUsersFromStats(ctx context.Context) ([]softether.HubUser, error) {
	stats, err := s.ListUserStats(ctx)
	if err != nil {
		return nil, err
	}
	out := make([]softether.HubUser, 0, len(stats))
	for _, st := range stats {
		out = append(out, softether.HubUser{
			Username:      st.Username,
			DownloadBytes: st.DownloadBytes,
			UploadBytes:   st.UploadBytes,
			LastIP:        st.ClientIP,
			LastISP:       st.ISP,
			TransferBytes: st.DownloadBytes + st.UploadBytes,
		})
	}
	return out, nil
}
...
func (s *Store) GetUserTrafficPeriodsMap(ctx context.Context) (map[string]UserTrafficPeriods, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&p.Username,
			&p.YesterdayBytes, &p.WeekBytes, &p.MonthBytes,
			&p.YesterdaySeconds, &p.WeekSeconds, &p.MonthSeconds,
		); err != nil {
			return nil, err
		}
		out[p.Username] = p
...
func (s *Store) LookupLatestByClientIP(ctx context.Context, ip string) (*SessionLog, error) {
	if ip == "" {
		return nil, ErrNotFound
	}
	var row SessionLog
	...
	err := s.pool.QueryRow(ctx, `
		...
		LIMIT 1
	`, ip).Scan(&row.Username, &row.DownloadBytes, &row.UploadBytes, &row.ClientIP, &rawISP,
		&row.DurationSeconds, &row.ConnectedAt, &row.DisconnectedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, ErrNotFound
		}
...
func (s *Store) ListOnlineSessions(ctx context.Context) ([]softether.OnlineSession, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT username, COALESCE(client_ip, ''), COALESCE(asn, ''), download_bytes, upload_bytes,
		       duration_seconds, connected_at, session_key
		FROM softether_sessions
		WHERE disconnected_at IS NULL
		ORDER BY username, connected_at DESC
	`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	out := make([]softether.OnlineSession, 0, 16)
	for rows.Next() {
		var row softether.OnlineSession
		var rawISP string
		var dl, ul int64
		var dur int64
		var connected time.Time
		if err := rows.Scan(
			&row.Username, &row.ClientIP, &rawISP, &dl, &ul, &dur, &connected, &row.SessionKey,
		); err != nil {
			return nil, err
		}
		row.LastISP = asn.OrgName(rawISP)
		row.DownloadBytes = uint64(dl)
		row.UploadBytes = uint64(ul)
		row.SessionDurationSeconds = dur
		t := connected.UTC()
		row.ConnectedAt = &t
		if row.SessionKey != "" {
			parts := strings.SplitN(row.SessionKey, "|", 2)
			if len(parts) == 2 {
				row.SessionName = parts[1]
			}
		}
		out = append(out, row)
	}
